...
The animation methods slideUp(), slideDown(), fadeIn() and fadeOut() have been removed from the Div tag. They should be replaced by [calling Animations.xxxx with the node to animate](../animations/index.md).
...
- Columns are now resizable with the mouse. Changes made by the user are sent to the server, so it is possible to persist those changes. See [the documentation](../domui-component-overview/the-datatable-component/index.md) for details.
...
- Column width length calculation has been fixed - [see the page for details](../domui-component-overview/the-datatable-component/index.md). This mostly fixes the issue where column sizes changed every time we used the pager.
...
The [new Tree2 component](the-tree2-component/index.md) is a rewrite of the now deprecated Tree component. It uses mostly the same interface but is rewritten with cleaner code and renders without using tables.
...
The generated html code for the controls has been changed so that [style fixes could be applied](lookupinput-lookupinput2-behavior-and-layout-rules/index.md). In addition, to reduce the maintenance of both LookupInput components, **both components now also share a common stylesheet** as they render the exact same layout. This breaks the existing legacy stylesheets! For fixes look at \_lookupInput.scss.
...
You can [use FontAwesome](fontawesome-support/index.md) icons in most places where you would usually add an image resource.

<a id="sass-scss-support"></a>

### Sass scss support

The main theme is now a scss theme. Support for SASS scss style sheets is [natively included in DomUI](../sass-scss-support/index.md): there is no need to translate scss to css, it is done by DomUI's embedded sass part.
