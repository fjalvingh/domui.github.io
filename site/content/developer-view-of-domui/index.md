...
- [The database layer](../the-generic-query-framework-qcriteria/index.md) is closely integrated in DomUI and generalized so that backend technologies like Hibernate, JPA or just plain JDBC are all possible using the same data interface.
...
- DomUI has an [abstract and generic query interface](../the-generic-query-framework-qcriteria/index.md), loosely modeled like Hibernate's Criteria, but without it's design flaws. By using this framework kludges like manually creating SQL are not needed in components that help with defining queries.
...
- The framework is Java-centric, and uses the [typeful properties](../using-typed-properties-the-property-annotations-processor/index.md) to create type-safe queries.
