...
- Since only delta's are sent DomUI is __fast__.
...
- This means: no need to write badly typed DTO's and error prone web services for the UI!
...
- Since Java's architects __still__ fail to solve the properties problem DomUI has a [concept of "typed properties"](../../data/data-binding/typed-properties/index.md). These are classes automatically generated by an apt compiler extension which contain all the properties and their type of data classes. Using typed properties makes the code again way more safe: any refactoring of property names will cause *compile time* errors instead of runtime problems. In addition, because the properties are typeful, a lot of casts are no longer necessary.
- DomUI has a database abstraction layer that is used throughout the code. Implementations for that layer exist for JPA and Hibernate. These abstractions, together with DomUI's component model, make things like SQL injection impossible when used properly. It also helps to defend against library changes: the major changes in different Hibernate versions had almost no effect on some of the huge applications DomUI is used in; only DomUI itself needed to implement the changes.  
...
- DomUI integrates out of the box with Hibernate 7.2
